    			...
    			<div class="card">
    				...
    				<div class="card-body">
                        @if ($errors->any())
                            ...
                            @endforeach
                        @endif
    					{{ Form::open(['url' => ['admin/config/permissions/'.$permissions->id.'/edit']]) }}
                            <div class="form-group">
                                {!! Form::label('name','Permission') !!}
                                {!! Form::text('name',$permissions->name,['class'=>'form-control col-4']) !!}
                            </div>
                            <div class="form-group">
                                ...
                                <div class="custom-controls-stacked">
                                    @foreach ($roles as $role) 
                                        <label class="custom-control custom-checkbox">
                                            {{ Form::checkbox('roles[]',  $role->id, (in_array($role->id, $ext_roles) ? true : false),['class'=>'custom-control-input'] ) }}
